export interface Hunt {
  ...
  const CACHES_KEY = "lootopia_caches"
  const USER_CACHES_KEY = "lootopia_user_caches"
  ...
  export class HuntStorage {
    static initializeStorage() {
      ...
      const existingCaches = localStorage.getItem(CACHES_KEY)
      if (!existingCaches) {
        localStorage.setItem(CACHES_KEY, JSON.stringify([]))
      }
  
      const existingUserCaches = localStorage.getItem(USER_CACHES_KEY)
      if (!existingUserCaches) {
        localStorage.setItem(USER_CACHES_KEY, JSON.stringify([]))
      }
    }
    ...
    static getAllCaches(): Cache[] {
      if (typeof window === "undefined") return []
      const caches = localStorage.getItem(CACHES_KEY)
      return caches ? JSON.parse(caches) : []
    }
  
    static getCachesByHuntId(huntId: string): Cache[] {
      return this.getAllCaches().filter((cache) => cache.huntId === huntId && cache.isActive)
    }
  
    static createCache(cacheData: Omit<Cache, "id" | "discovered" | "isActive">): Cache {
      ...
      localStorage.setItem(CACHES_KEY, JSON.stringify(caches))
  
      return newCache
    }
  
    static updateCache(id: string, updates: Partial<Cache>): Cache | null {
      const caches = this.getAllCaches()
      const cacheIndex = caches.findIndex((cache) => cache.id === id)
  
      if (cacheIndex === -1) return null
  
      caches[cacheIndex] = { ...caches[cacheIndex], ...updates }
      localStorage.setItem(CACHES_KEY, JSON.stringify(caches))
    ...
    static deleteCache(id: string): boolean {
      const caches = this.getAllCaches()
      const filteredCaches = caches.filter((cache) => cache.id !== id)
  
      if (filteredCaches.length === caches.length) return false
  
      localStorage.setItem(CACHES_KEY, JSON.stringify(filteredCaches))
      return true
    }
  
    static discoverCache(cacheId: string, userId = "current_user"): boolean {
      const cache = this.getAllCaches().find((c) => c.id === cacheId)
      if (!cache || cache.discovered) return false
  
      // Marquer le cache comme découvert
      ...
      // Ajouter à l'inventaire utilisateur
      const userCaches = this.getUserCaches()
      ...
      userCaches.push(newUserCache)
      localStorage.setItem(USER_CACHES_KEY, JSON.stringify(userCaches))
  
      return true
    }
  
    static getUserCaches(): UserCache[] {
      if (typeof window === "undefined") return []
      const userCaches = localStorage.getItem(USER_CACHES_KEY)
      return userCaches ? JSON.parse(userCaches) : []
    }
  
    static getUserCachesByHunt(huntId: string): UserCache[] {
      return this.getUserCaches().filter((cache) => cache.huntId === huntId)
    }
  
    static findNearbyCache(huntId: string, latitude: number, longitude: number, radius = 0.001): Cache | null {
      const caches = this.getCachesByHuntId(huntId).filter((cache) => !cache.discovered)
  
      for (const cache of caches) {
        const distance = Math.sqrt(Math.pow(cache.latitude - latitude, 2) + Math.pow(cache.longitude - longitude, 2))
